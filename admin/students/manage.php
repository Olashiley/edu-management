	...
	<div class="row">
		...
		<div class="col-md-6">
			<br>
				...
				<input type="text" class="form-control form-control-sm" name="address" id="address" value="<?php echo isset($address) ? $address : '' ?>" required>
			</div>
		...
		$('#faculty-frm').submit(function(e){
			...
			$.ajax({
				...
				if(resp == 1){
					...
					var _msg = "<div class='alert alert-danger text-white err_msg'><i class='fa fa-exclamation-triangle'></i> Studemt already exists.</div>"
					_frm.prepend(_msg)
