...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM home where id = {$_GET['id']}");
	...
	$class_arr = array();
	$qry2 = $conn->query("SELECT * FROM home_class where home_id = {$_GET['id']}");
...
<div class="card card-outline card-primary">
		...
		<div class="container-fluid">
			<form action="" id="manage-lesson">
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="title" value="<?php echo isset($title) ? $title : "" ?>" required="" readonly>
				</div>
				...
				<div class="form-group">
					...
					<input type="text" class="form-control" name="description" value="<?php echo isset($description) ? $description : "" ?>" required="" readonly>
				</div>
                <div class="form-group">
					<label for="" class="control-label">Questions</label>
                    <label for="" class="control-label"><?php echo isset($question) ? $question : "";?></label>
				</div>
                <div class="form-group">
					<label for="" class="control-label">Answers</label>
		             <textarea name="answer" id="" cols="300" rows="100" class="form-control summernote"><?php echo isset($answer) ? $answer : ""; ?></textarea>
				</div>
                <div class="form-group">
                    <label for="" class="control-label">Upload Assignment Files (PDF, DOCX, Videos)</label>
                    <input type="file" class="form-control" name="home_file[]" multiple accept=".pdf, .docx, .mp4, .mov, .avi">
                </div>
		</form>
		</div>
	</div>
	<div class="card-footer">
		<div class="col-md-12">
			...
			<a type="cutton" class="btn btn-flat btn-default" href="./?page=homework">Cancel</a>
		</div>
	</div>
</div>
<script>
	$(document).ready(function(){
		$('.select2').select2();
		 ...
		 $('#manage-lesson').submit(function(e){
		 	...
		 	start_loader();
		 	$.ajax({
				url:_base_url_+'classes/Master.php?f=save_home',
				...
				success:function(resp){
					if(resp>0){
						location.href = "./?page=homework/view_assingnment&id="+resp;
					}else{
						alert_toast('An error occured');
						end_loader();
					}
